...
def main():
    ...
    features = {}
    features['raw'] = len(X_train.columns)
    ...
    features['engineered'] = len(X_train.columns)
    ...
    features['tfidf_text'] = len(X_train.columns)
    ...
    features['tfidf_pos'] = len(X_train.columns)
...
def tf_idf_matrix(df, column, tfidfvectorizer):
    '''
    Takes a DataFrame, a column of text, and  tfidfVectorizer. Creates tf-idf
    ...
    X_train_tfidf = tfidf.fit_transform(df[column])
